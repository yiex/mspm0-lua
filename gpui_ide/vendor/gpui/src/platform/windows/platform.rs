...
impl WindowsPlatform {
    pub(crate) fn new() -> Result<Self> {
        ...
        let text_system = Arc::new(DirectWriteTextSystem::new(&directx_devices).with_context(
            || {
                "Error creating DirectWriteTextSystem (GPU/D3D11 or DWrite/shader init). \
                 Needs Win10 1703+, GPU drivers, d3dcompiler_47.dll. \
                 If this is a debug build shared to another PC, rebuild after shader embed fix."
            },
        )?);
