...
pub(crate) mod shader_resources {
    ...
    pub(super) fn build_shader_blob(entry: ShaderModule, target: ShaderTarget) -> Result<ID3DBlob> {
        // Embed HLSL so debug binaries work on other machines (no CARGO_MANIFEST_DIR path).
        const ALPHA: &str = include_str!("alpha_correction.hlsl");
        const SHADERS: &str = include_str!("shaders.hlsl");
        const COLOR_TEXT: &str = include_str!("color_text_raster.hlsl");
        ...
        unsafe {
            ...
            // Expand local #include "alpha_correction.hlsl" so we need no file-system includes.
            let source = source.replace("#include \"alpha_correction.hlsl\"", ALPHA);
            let source = source.replace("#include \"alpha_correction.hlsl\"", ALPHA);
            ...
            if ret.is_err() {
                let Some(error_blob) = error_blob else {
                    return Err(anyhow::anyhow!(
                        "D3DCompile failed ({ret:?}). Need d3dcompiler_47.dll (Windows 10+)."
                    ));
                };
